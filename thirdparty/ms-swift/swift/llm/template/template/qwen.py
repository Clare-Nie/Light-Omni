...
class Qwen2_5OmniTemplate(Qwen2_5VLTemplate):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        encoded['loss_scale'] = loss_scale
        encoded.update(media_inputs)
        encoded["meta"] = inputs.extra_kwargs["meta"]
        return encoded
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        token_keep_mask = self.get_compression_mask(
    ...
    def get_compression_mask(self, inputs_embeds, input_ids, image_grid_thw, image_token_id, 
                            spatial_merge_size=2, keep_ratio=0.25, inter_thresh=0.96):
        """
        在 _get_inputs_embeds_hf 之后调用，计算需要保留的 Token 索引。
        """
        device = inputs_embeds.device
        # 1. 初始化全保留 Mask
        global_mask = torch.ones(inputs_embeds.shape[1], dtype=torch.bool, device=device)
        
        if image_grid_thw is None:
            return global_mask

        # 2. 定位所有图片 Token (input_ids 此时是全长的)
        img_loc_mask = (input_ids == image_token_id) # (1, Seq_Len)
        if not img_loc_mask.any():
            return global_mask

        all_img_feats = inputs_embeds[0][img_loc_mask[0]]
        
        # 3. 遍历 Grid 计算
        # 此时 grid 是原始尺寸，需要除以 merge_size 得到 feature map 尺寸
        curr_grid = image_grid_thw.clone()
        curr_grid[:, 1:] //= spatial_merge_size
        ...
        # 用于收集图片区域局部的 mask
        img_keep_bits = []
        total_frames = len(curr_grid)
        dropped_frames = 0
        
        for t, h, w in curr_grid:
            L = h * w
            feat = all_img_feats[start : start + L].view(h, w, -1)
            # --- A. 帧间压缩 (Drop Frame) ---
            curr_mean = feat.mean((0, 1)).detach()
            
            # 【修改点1】：默认 k 值逻辑 (正常帧保底32)
            k = max(32, int(L * keep_ratio))
            
            is_dropped = False
            if prev_mean is not None:
                if F.cosine_similarity(curr_mean.unsqueeze(0), prev_mean.unsqueeze(0)) > inter_thresh:
                    # 【修改点2】：如果是要 drop 的帧，不直接 continue，而是将 k 强制设为 32
                    is_dropped = True
                    dropped_frames += 1
                    k = 32 
            
            # 只有未被判为“丢弃”的帧才更新 prev_mean，保证相似度对比的连贯性
            if not is_dropped:
                prev_mean = curr_mean
            
            feat_flat = feat.view(L, -1)
            feat_norm = F.normalize(feat_flat, p=2, dim=-1) # (L, C)
            mean_norm = F.normalize(curr_mean, p=2, dim=-1).unsqueeze(0) # (1, C)
            similarity = torch.mm(feat_norm, mean_norm.t()).squeeze(1)
            ...
            m[indices] = True
            img_keep_bits.append(m)
            start += L

        # 4. 将计算出的局部 Mask 填回全局 Mask
        if img_keep_bits:
            flat_img_bits = torch.cat(img_keep_bits)
            # img_loc_mask[0] 是所有图片的位置，将其替换为计算出的 bits
            global_mask[img_loc_mask[0]] = flat_img_bits
        
        print(
            f"[Compression Summary] "
            f"Frames Dropped (Compressed to 32): {dropped_frames}/{total_frames}. "
            f"Image Token Keep Ratio: {flat_img_bits.sum().item() / img_loc_mask.sum().item():.3f} (current: {flat_img_bits.sum().item()}). "
            f"Overall Keep Ratio: {global_mask.sum().item() / len(global_mask):.3f}."
            f" Len {len(global_mask)} keep_ratio {keep_ratio} inter_thresh {inter_thresh} 111"
        )
        return global_mask
