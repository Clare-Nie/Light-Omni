...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        # 设置额外的latent embedding 用于计算，包括响应、检索和检索向量
        # training stage1 需要调用_load_emb_model
        from transformers import AutoModelForCausalLM, AutoTokenizer
        self.emb_model = AutoModelForCausalLM.from_pretrained(
            "Qwen/Qwen3-Embedding-0.6B",
            torch_dtype=torch.bfloat16,
            trust_remote_code=True,
            attn_implementation="flash_attention_2"
        )
        self.emb_tokenizer = AutoTokenizer.from_pretrained('Qwen/Qwen3-Embedding-0.6B', padding_side='left')
        self.latent_size = 32
        self.latent_is_response = nn.Parameter(
            torch.randn(1, self.latent_size, self.model.config.hidden_size, dtype=torch.bfloat16)
        )
        self.latent_is_retrieve = nn.Parameter(
            torch.randn(1, self.latent_size, self.model.config.hidden_size, dtype=torch.bfloat16)
        )
        self.latent_retrieval_vector = nn.Parameter(
            torch.randn(1, self.latent_size, self.model.config.hidden_size, dtype=torch.bfloat16)
        )
        self.hidden_to_emb = nn.Sequential(
            nn.Linear(self.model.config.hidden_size, self.emb_model.config.hidden_size),
            nn.ReLU(),
            nn.Linear(self.emb_model.config.hidden_size, self.emb_model.config.hidden_size)
        )
        self.loss_weights = {
            "retrieve_r1": 0.5,
            "retrieve_r2": 0.5,
    ...
    def start_feature_cache(self):
        ...
        self.image_fea_cache = {}
        self.audio_fea_cache = {}
        self.comp_mask_cache = {}
    ...
    def _get_cache_key(self, tensor):
        ...
        return f"{tensor.shape}-{tensor[0].sum().item():.3f}-{tensor[-1].sum().item():.3f}"
    ...
    def load_emb_model(self):
        ...
        for param in self.emb_model.parameters():
            param.requires_grad = False
        for module in [self.hidden_to_emb]:
            for layer in module:
        ...
        nn.init.normal_(self.latent_is_response, std=0.02)
        nn.init.normal_(self.latent_is_retrieve, std=0.02)
        nn.init.normal_(self.latent_retrieval_vector, std=0.02)
    
    
    def get_emb_input_embedding_from_text(self, texts, is_query=False, soft_prompt=None, random_drop=False):
        if not isinstance(texts, list):
            texts = [texts]
        
        is_dropped = False
        if is_query and random_drop and self.training and torch.rand(1).item() < 0.9:
            texts = [""] * len(texts)
            is_dropped = True
        
        if is_query and not is_dropped:
        ...
        for i in range(0, len(texts), batch_size_limit):
            ...
            batch_dict = self.emb_tokenizer(
                batch_texts,
                max_length=8192,
                padding=True,
                truncation=True,
                return_tensors='pt'
            ).to(self.emb_model.device)
            ...
            if is_query and soft_prompt is not None:
                curr_soft_prompt = soft_prompt.to(device=text_emb.device, dtype=text_emb.dtype)
                if curr_soft_prompt.dim() == 2:
                    curr_soft_prompt = curr_soft_prompt.unsqueeze(0)

                prompt_vec = curr_soft_prompt.mean(dim=1)
                
                if prompt_vec.shape[0] == 1 and text_emb.shape[0] > 1:
                    prompt_vec = prompt_vec.expand(text_emb.shape[0], -1)
                elif prompt_vec.shape[0] == len(texts):
                    prompt_vec = prompt_vec[i : i + batch_size_limit]

                text_norm = torch.nn.functional.normalize(text_emb, p=2, dim=1)
                prompt_norm = torch.nn.functional.normalize(prompt_vec, p=2, dim=1)
                
                if is_dropped:
                    batch_emb = prompt_norm
                else:
                    batch_emb = text_norm + prompt_norm
            else:
                batch_emb = text_emb
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        ...
        if self.feature_cache:
            cache_key = self._get_cache_key(pixel_values)
            if cache_key in self.image_fea_cache:
                torch.cuda.synchronize()
                # print(f"image feature cache hit time: {time.time() - time1:.3f}", f"cache hit time: {time.time() - time1:.3f}")
                return self.image_fea_cache[cache_key]
        ########
        ...
        ########
        if self.feature_cache and cache_key is not None:
            self.image_fea_cache[cache_key] = image_embeds.detach()
        ########
        return image_embeds
    ...
    def get_audio_features(
        ...
        #######
        if self.feature_cache:
            cache_key = self._get_cache_key(input_features)
            if cache_key in self.audio_fea_cache:
                # torch.cuda.synchronize()
                # print("audio feature cache hit", f"cache hit time: {time.time() - time1:.3f}")
                return self.audio_fea_cache[cache_key]
        ########
        ...
        ########
        if self.feature_cache and cache_key is not None:
            self.audio_fea_cache[cache_key] = audio_features.detach()
        ########
    ...
    def get_compression_mask(self, inputs_embeds, input_ids, image_grid_thw, image_token_id, 
        ...
        #######
        if self.feature_cache:
            cache_key = self._get_cache_key(all_img_feats)
            if cache_key in self.comp_mask_cache:
                # print("compression feature cache hit")
                global_mask[img_loc_mask[0]] = self.comp_mask_cache[cache_key]
                return global_mask
        #######
        ...
        if img_keep_bits:
            ...
            if self.feature_cache and cache_key is not None:
                self.comp_mask_cache[cache_key] = flat_img_bits.detach()
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if pixel_values is not None:
            ...
            if token_keep_mask is None:
                token_keep_mask = self.get_compression_mask(
                    inputs_embeds, input_ids, image_grid_thw, self.config.image_token_id,
        ...
        if token_keep_mask is not None:
            self.last_token_keep_mask = token_keep_mask
            inputs_embeds = inputs_embeds[:, token_keep_mask].contiguous()
            if position_ids is not None:
                position_ids = position_ids[:, :, token_keep_mask].contiguous()
            if attention_mask is not None:
                attention_mask = attention_mask[:, token_keep_mask].contiguous()
            if labels is not None:
                labels = labels[:, token_keep_mask].contiguous()
            new_seq_len = inputs_embeds.shape[1]
            self.real_kv_len = new_seq_len
            if 'cu_seq_lens_q' in kwargs:
                # 假设 Batch=1，构造新的 cumulative sequence lengths
                new_cu_seqlens = torch.tensor([0, new_seq_len], dtype=torch.int32, device=inputs_embeds.device)
                kwargs['cu_seq_lens_q'] = new_cu_seqlens
                kwargs['cu_seq_lens_k'] = new_cu_seqlens
                kwargs['max_length_q'] = new_seq_len
                kwargs['max_length_k'] = new_seq_len
            if cache_position is not None:
                cache_position = cache_position[: new_seq_len]
        elif hasattr(self, "real_kv_len") and inputs_embeds.shape[1] == 1:
            cache_position = torch.tensor([self.real_kv_len], device=inputs_embeds.device)
            attention_mask = torch.ones((1, self.real_kv_len + 1), device=inputs_embeds.device, dtype=torch.long)
            self.real_kv_len += 1

        retrieve_type = None
        info = None
        latents_to_append = None
        extra_len = 0
        inference_mode = False

        def run_model_forward(curr_inputs_embeds, curr_attention_mask, curr_position_ids, curr_kwargs):
            return self.model(
        ...
        if meta:
            ...
            _type = meta_item["type"]
            self.model.set_adapter(_type)
            inference_mode = meta_item.get("inference_mode", False)
            if _type == "retrieve":
                info = meta_item["info"]
                retrieve_type = info.get("type", None)
                latents_to_append = torch.cat([
                        self.latent_is_response,
                        self.latent_is_retrieve,
                        self.latent_retrieval_vector
                    ], dim=1)
                
                if not inference_mode and retrieve_type in ["retrieve_r1", "retrieve_r2", "retrieve_r3"]:
                    inputs_embeds = inputs_embeds[:, :-3, :]
                    attention_mask = attention_mask[..., :-3]
                    position_ids = position_ids[..., :-3]

                if latents_to_append is not None:
                    inputs_embeds = torch.cat([inputs_embeds, latents_to_append], dim=1)
                    extra_len = latents_to_append.shape[1]
                    new_mask = torch.ones(
                    ...
                    if position_ids is not None:
                        max_pos = position_ids[0, 0, -1] if position_ids.dim() == 3 else position_ids[0, -1]
                        new_pos = torch.arange(max_pos + 1, max_pos + 1 + extra_len, device=position_ids.device)
                        if position_ids.dim() == 3:
                            new_pos = new_pos.view(1, 1, -1).expand(3, 1, -1)
                            position_ids = torch.cat([position_ids, new_pos], dim=2)
                        else:
                            new_pos = new_pos.unsqueeze(0).expand(1, -1)
                            position_ids = torch.cat([position_ids, new_pos], dim=1)

                    if inference_mode:
                        cache_position = None
                    elif cache_position is not None:
                        last_pos = cache_position[-1]
                        new_cache_pos = torch.arange(
                            last_pos + 1, last_pos + 1 + extra_len, 
                            device=cache_position.device, 
                            dtype=cache_position.dtype
                        )
                        cache_position = torch.cat([cache_position, new_cache_pos], dim=0)

                    if 'cu_seq_lens_q' in kwargs:
                        actual_seq_len = inputs_embeds.shape[1]
                        new_cu_seq = torch.tensor([0, actual_seq_len], dtype=torch.int32, device=inputs_embeds.device)
                        kwargs['cu_seq_lens_q'] = new_cu_seq
                        kwargs['cu_seq_lens_k'] = new_cu_seq
                        kwargs['max_length_q'] = actual_seq_len
                        kwargs['max_length_k'] = actual_seq_len

        outputs = run_model_forward(inputs_embeds, attention_mask, position_ids, kwargs)
        hidden_states = outputs[0]
        loss = None

        if inference_mode:
            is_response_logits = self.lm_head(hidden_states[:, -(2 * self.latent_size + 1), :]).argmax(dim=-1)
            is_retrieve_logits = self.lm_head(hidden_states[:, -(self.latent_size + 1), :]).argmax(dim=-1)
            soft_prompt = self.hidden_to_emb(hidden_states[:, -self.latent_size:, :])
            return {
                "is_response": True if is_response_logits[0] == 9693 else False,
                "is_retrieve": True if is_retrieve_logits[0] == 9693 else False,
                "soft_prompt": soft_prompt,
            }

        if meta and retrieve_type:
            if retrieve_type == "retrieve_r1":
                logits = self.lm_head(hidden_states[:, -(2 * self.latent_size + 1), :])
            elif retrieve_type == "retrieve_r2":
                logits = self.lm_head(hidden_states[:, -(self.latent_size + 1), :])
            if retrieve_type in ["retrieve_r1", "retrieve_r2"]:
                if labels is not None:
                    target_labels = labels[:, -3] # [Batch]
                    loss_fct = torch.nn.CrossEntropyLoss()
                    loss = loss_fct(logits, target_labels) * self.loss_weights.get(retrieve_type, 1.0)
                    print("retrieve_type ", retrieve_type, "loss ", loss, labels[:, -3:])
            elif retrieve_type == "retrieve_r3":
                logits = self.lm_head(hidden_states[:, -self.latent_size:, :])
                latent_hiddens = hidden_states[:, -self.latent_size:, :]
                soft_prompt = self.hidden_to_emb(latent_hiddens)
                query_texts = info["query"] if isinstance(info["query"], list) else [info["query"]]
                query_embs = self.get_emb_input_embedding_from_text(
                    query_texts, 
                    is_query=True,
                    soft_prompt=soft_prompt,
                    random_drop=True
                )
                content_embs = self.get_emb_input_embedding_from_text(
                    info["content"], 
                    is_query=False
                )
                pos_indices = info["indices"]
                num_docs = content_embs.shape[0]
                is_pos = torch.zeros(num_docs, dtype=torch.bool, device=content_embs.device)
                is_pos[pos_indices] = True
                pos_embs = content_embs[is_pos].unsqueeze(0)   # [1, Num_Pos, D]
                neg_embs = content_embs[~is_pos].unsqueeze(0)  # [1, Num_Neg, D]
                query_emb_norm = query_embs
                pos_scores = torch.bmm(query_emb_norm.unsqueeze(1), pos_embs.transpose(1, 2)).squeeze(1) / 0.01
                neg_scores = torch.bmm(query_emb_norm.unsqueeze(1), neg_embs.transpose(1, 2)).squeeze(1) / 0.01
                loss_fct = torch.nn.CrossEntropyLoss()
                total_loss = 0.0
                # For each positive sample, calculate InfoNCE-like loss treating it as the target against all negatives
                for i in range(pos_scores.shape[1]):
                    # Logits: [Positive_i, Neg_1, Neg_2, ...]
                    current_logits = torch.cat([pos_scores[:, i:i+1], neg_scores], dim=1) # [1, 1+Negs]
                    target = torch.zeros(current_logits.shape[0], dtype=torch.long, device=query_emb_norm.device)
                    total_loss += loss_fct(current_logits, target)
                loss = total_loss / pos_scores.shape[1] * self.loss_weights.get(retrieve_type, 1.0)
                print("retrieve_type ", retrieve_type, "loss ", loss, pos_scores, neg_scores)

            # Add dummy loss avoids stucking
            dummy_loss = 0.0
            for p in [self.latent_is_response, self.latent_is_retrieve, self.latent_retrieval_vector]:
                dummy_loss += p.sum()
            for p in self.hidden_to_emb.parameters():
                dummy_loss += p.sum()
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        training_stage1 = False
        if not training_stage1:
            ...
            self.thinker.model = get_peft_model(self.thinker.model, peft_config_response, adapter_name="response")
            self.thinker.model.add_adapter("memory", peft_config_memory)
            self.thinker.model.add_adapter("retrieve", peft_config_retrieve)
            print("LoRA adapter 'memory' and 'response' and 'retrieve' added")
    ...
    def generate(
        ...
        **kwargs,
        ...
        thinker_kwargs["meta"] = meta
        thinker_result = self.thinker.generate(input_ids=input_ids, **thinker_kwargs)

        if not generate_audio:
            return thinker_result
        
        if hasattr(self.thinker, "last_token_keep_mask"):
            keep_mask = self.thinker.last_token_keep_mask
            input_ids = input_ids[:, keep_mask]
            if "attention_mask" in kwargs:
                kwargs["attention_mask"] = kwargs["attention_mask"][:, keep_mask]
            
            img_token_id = self.config.thinker_config.image_token_id
            is_img = (input_ids[0] == img_token_id)
            
            diff = is_img.int().diff(prepend=torch.tensor([0], device=input_ids.device))
            starts = (diff == 1).nonzero().flatten()
            ends = (diff == -1).nonzero().flatten()
            if len(ends) < len(starts): 
                ends = torch.cat([ends, torch.tensor([len(is_img)], device=input_ids.device)])
            
            lengths = ends - starts # 压缩后每张图保留的 Token 数
            if len(lengths) > 0:
                talker_kwargs["image_grid_thw"] = torch.stack([
                    lengths,
                    torch.full_like(lengths, 2), 
                    torch.full_like(lengths, 2)
                ], dim=1)
