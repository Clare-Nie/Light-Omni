...
class SwiftSft(SwiftPipeline, TunerMixin):
    ...
    def run(self):
        ...
        if not isinstance(self.model.thinker.model, PeftModel):
            peft_config_response = LoraConfig(
            ...
            peft_config_retrieve = LoraConfig(
                inference_mode=False,
                r=64, 
                lora_alpha=128, 
                lora_dropout=0.05,
            ...
            self.model.thinker.model = get_peft_model(self.model.thinker.model, peft_config_response, adapter_name="response")
            print("LoRA adapter 'response' added.")
            self.model.thinker.model.add_adapter("memory", peft_config_memory)
            print("LoRA adapter 'memory' added.")
            self.model.thinker.model.add_adapter("retrieve", peft_config_retrieve)
            print("LoRA adapter 'retrieve' added.")
            self.model.thinker.load_emb_model()
        else:
            if "response" in self.model.thinker.model.peft_config:
                print("LoRA adapter 'response' already exists, skipping.")
            else:
                print("Model is already a PeftModel but 'response' adapter is missing.")
            if "memory" in self.model.thinker.model.peft_config:
                print("LoRA adapter 'memory' already exists, skipping.")
            else:
                print("Model is already a PeftModel but 'memory' adapter is missing.")
            if "retrieve" in self.model.thinker.model.peft_config:
                print("LoRA adapter 'retrieve' already exists, skipping.")
            else:
                print("Model is already a PeftModel but 'retrieve' adapter is missing.")
        logger.info(f'model lora: {self.model}')
        ################

        print("=================sft=========================")
        training_stage = args.training_stage
        for name, param in self.model.thinker.model.named_parameters():
            if 'lora' in name and "response" in name and training_stage == 'stage1':
                param.requires_grad = True
            elif 'lora' in name and "memory" in name and training_stage == 'stage2':
                param.requires_grad = True
            elif 'lora' in name and "retrieve" in name and training_stage == 'stage3':
                param.requires_grad = True
            else:
                param.requires_grad = False
            ###
        for name, param in self.model.thinker.lm_head.named_parameters():
            param.requires_grad = False
        for name, param in self.model.thinker.named_parameters():
            if "hidden_to_emb" in name or "latent_is_response" in name or "latent_is_retrieve" in name or "latent_retrieval_vector" in name:
                if training_stage == 'stage3':
                    param.requires_grad = True
                else:
                    param.requires_grad = False
        for name, param in self.model.thinker.emb_model.named_parameters():        
            param.requires_grad = False
